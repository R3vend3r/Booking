...
class BookingControllerTest {
    ...
    void createBooking_shouldReturnCreatedWhenSuccess() throws Exception {
        ...
        BookingResponse response = createFullBookingResponse("B-123", start, end, "CLIENT-1", "John Doe", "WP-1", "Workplace 1");
    ...
    @Test
    void createBooking_shouldReturnNotFoundWhenClientNotFound() throws Exception {
        LocalDateTime start = LocalDateTime.now().plusDays(1);
        LocalDateTime end = start.plusHours(2);
    ...
    @Test
    void createBooking_shouldReturnNotFoundWhenWorkPlaceNotFound() throws Exception {
        LocalDateTime start = LocalDateTime.now().plusDays(1);
        LocalDateTime end = start.plusHours(2);
    ...
    void getBookingById_shouldReturnBookingWhenFound() throws Exception {
        ...
        LocalDateTime start = LocalDateTime.now().plusDays(1);
        LocalDateTime end = start.plusHours(2);
        BookingResponse response = createFullBookingResponse(bookingId, start, end, "CLIENT-1", "John Doe", "WP-1", "Workplace 1");
    ...
    void getAllBookings_shouldReturnListOfBookings() throws Exception {
        ...
        List<BookingResponse> bookings = Arrays.asList(
                createFullBookingResponse("B-123", start1, end1, "CLIENT-1", "John Doe", "WP-1", "Workplace 1"),
                createFullBookingResponse("B-456", start2, end2, "CLIENT-2", "Jane Smith", "WP-2", "Workplace 2")
        );
    ...
    void getBookingsByClient_shouldReturnBookingsWhenClientExists() throws Exception {
        ...
        List<BookingResponse> bookings = Arrays.asList(
                createFullBookingResponse("B-123", start, end, clientId, "John Doe", "WP-1", "Workplace 1")
        );
    ...
    void getBookingsByWorkPlace_shouldReturnBookingsWhenWorkPlaceExists() throws Exception {
        ...
        List<BookingResponse> bookings = Arrays.asList(
                createFullBookingResponse("B-123", start, end, "CLIENT-1", "John Doe", workPlaceId, "Workplace 1")
        );
        ...
        mockMvc.perform(get("/api/bookings/workplace/{workPlaceId}", workPlaceId)
                ...
                .andExpect(jsonPath("$[0].workPlaceName").value("Workplace 1"));
    ...
    void getActiveBookings_shouldReturnListOfActiveBookings() throws Exception {
        ...
        List<BookingResponse> bookings = Arrays.asList(
                createFullBookingResponse("B-123", start, end, "CLIENT-1", "John Doe", "WP-1", "Workplace 1")
        );
    ...
    void getBookingsByDateRange_shouldReturnBookings() throws Exception {
        ...
        List<BookingResponse> bookings = Arrays.asList(
                createFullBookingResponse("B-123", start, start.plusHours(2), "CLIENT-1", "John Doe", "WP-1", "Workplace 1")
        );
    ...
    void updateBooking_shouldUpdateSuccessfully() throws Exception {
        ...
        BookingResponse response = createFullBookingResponse(bookingId, start, end, "CLIENT-1", "John Doe", "WP-2", "Workplace 2");
        ...
        mockMvc.perform(put("/api/bookings/{id}", bookingId)
                ...
                .andExpect(jsonPath("$.workPlaceName").value("Workplace 2"));
    ...
    void checkWorkplaceAvailability_shouldReturnTrueWhenAvailable() throws Exception {
        ...
        when(bookingService.isWorkplaceOccupied(eq(workPlaceId), eq(start), eq(end), isNull()))
                .thenReturn(false);
        ...
        verify(bookingService, times(1)).isWorkplaceOccupied(eq(workPlaceId), eq(start), eq(end), isNull());
    }
    ...
    @Test
    void checkWorkplaceAvailability_shouldReturnTrueWhenAvailableWithExcludeBooking() throws Exception {
        String workPlaceId = "WP-1";
        String excludeBookingId = "B-123";
        LocalDateTime start = LocalDateTime.now().plusDays(1);
        LocalDateTime end = start.plusHours(2);

        when(bookingService.isWorkplaceOccupied(eq(workPlaceId), eq(start), eq(end), eq(excludeBookingId)))
                .thenReturn(false);

        mockMvc.perform(get("/api/bookings/workplace/{workPlaceId}/check", workPlaceId)
                        .param("start", start.toString())
                        .param("end", end.toString())
                        .param("excludeBookingId", excludeBookingId)
                        .contentType(MediaType.APPLICATION_JSON)
                        .accept(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(content().string("true"));

        verify(bookingService, times(1)).isWorkplaceOccupied(eq(workPlaceId), eq(start), eq(end), eq(excludeBookingId));
    }

    // Helper method to create full BookingResponse
    private BookingResponse createFullBookingResponse(String id, LocalDateTime start, LocalDateTime end,
        ...
        response.setWorkPlaceId(workPlaceId);
        response.setWorkPlaceName(workPlaceName);
        response.setLocationName("Main Office");
        response.setLocationAddress("123 Main St");
        response.setLocationCity("Moscow");
        response.setTotalAmount(5000L);
